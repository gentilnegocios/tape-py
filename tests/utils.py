...
import pytape.client
import pytape.transport
...
def get_client_and_http():
    """
    Gets a pytape.client.Client instance and a mocked instance of
    httplib2.Http that backs it. Returned as (client, Http)
    """
    transport = pytape.transport.HttpTransport(URL_BASE, headers_factory=dict)
    client = pytape.client.Client(transport)
