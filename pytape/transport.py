...
class BearerToken(object):
    """
    Class used to encapsulate the Bearer token required to access the
    Tape API.
    """
    ...
    def to_headers(self):
        return {'authorization': f"Bearer {self.user_key}"}


class BearerAuthorization(object):
    """Generates headers for Bearer Tape Authorization"""

    def __init__(self, user_key):

        self._user_key = user_key

        self.token = BearerToken(self._user_key)
